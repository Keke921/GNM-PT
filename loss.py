...
class GCLLoss(nn.Module):
    ...
    def __init__(self, config, cls_num_list=None):  #, cls_num_list, m=0.5, weight=None, s=30, train_cls=False, noise_mul = 1., gamma=0.
        super(GCLLoss, self).__init__()
        cls_list = torch.tensor(cls_num_list, dtype= torch.float32, device='cuda')
        m_list = torch.log(cls_list)
        m_list = m_list.max()-m_list
        self.m_list = m_list
        self.m = config.loss.GCL.m
        self.s = config.loss.GCL.s
        #self.weight = weight
        self.simpler = normal.Normal(0, 1/3)
        self.focal = config.loss.GCL.focal
        self.noise_mul = config.loss.GCL.noise_mul
        self.gamma = config.loss.GCL.gamma
        self.reweight_epoch  = config.loss.GCL.reweight_epoch
        if self.reweight_epoch != -1:
            #self.focal=True
            idx = 1 # condition could be put in order to set idx
            betas = [0, 0.9998]
    ...
    def forward(self, cosine, target):
        ...
        if self.focal:
            return focal_loss(F.cross_entropy(self.s*output, target, reduction='none', 
                                              weight=self.per_cls_weights), self.gamma)
        else:    
            return F.cross_entropy(self.s*output, target, weight=self.per_cls_weights) 
